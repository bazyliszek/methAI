...
def plot(lines, saveTo, title):
    ...
    ax.legend(legends, bbox_to_anchor=(0.85,0.25), bbox_transform=plt.gcf().transFigure)
...
'''
colorList = ['r','b','g','y']
filePath = "/home/fruitdragon/workspace/NeuralClassifier/old/data/PCA"
...
neuralNets = [[7,4], [10,10], [5,4,3], None]


for organType in ["bladder", "kidney", "prostate"]:
    for pcaNum in [2,4,8,16]:
        with open(os.path.join(filePath, organType, "cancer", organType+"Cancer_full_PCA"+str(pcaNum)+".pkl"), 'rb') as cancerFile:
            cancer = pickle.load(cancerFile)
        with open(os.path.join(filePath, organType, "healthy", organType+"Healthy_full_PCA"+str(pcaNum)+".pkl"), 'rb') as healthyFile:
            healthy= pickle.load(healthyFile)
        
        lines = []
        for neuralNet in neuralNets:
            if(neuralNet == None):
                sens, negSpec = getRandomRocPoints(cancer, healthy)
                name = "Random"
            else:
                sens, negSpec = getRocPoints(cancer, healthy, neuralNet, netIterations, includeTraining)
                name = "NN(" +str(neuralNet) + ")"
                    
            AUC = areaUnderCurve(sens, negSpec)
            lines.append(RocLine(name, sens, negSpec, AUC))
        
        #plot(lines, None, organType+"_PCA"+str(pcaNum))
        plot(lines, "/home/fruitdragon/workspace/NeuralClassifier/old/images", organType+"_PCA"+str(pcaNum))
